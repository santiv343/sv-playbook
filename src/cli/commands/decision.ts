...
function handleAnswer(args: string[], io: Io): number {
  ...
  return withStore((store, repoRoot) => {
    // ⚠️ Ver findings.md F-006: este chequeo exige que .svp-session-role
    // EXISTA y diga literalmente 'human'. El modelo documentado en
    // content/cli.md es el opuesto — el archivo es identidad
    ...
    // sesión humana normal (sin el archivo) probablemente cae en este
    // rechazo. No corregido acá — es una decisión de producto, no un
    // fix mecánico obvio.
