...
**Pre-flight — flush open threads (this closes the "how does it know what we were doing" gap):** BEFORE printing the prompt, handoff inspects every `active` and `blocked` packet. For each one whose newest `note` event is OLDER than its newest `transition` event (i.e. it changed state but nobody recorded where it stands), print a blocking WARNING to stderr listing those packet ids and instructing the outgoing agent to run `sv-playbook task note <id> "<where I left off>"` first, then re-run handoff. Rationale: handoff can only transmit what is written to the board — it does NOT and CANNOT read the chat/session. The warning forces the outgoing agent to persist its in-flight intent so the next agent truly continues "exactly the same". A `--force` flag skips the warning for a genuinely clean board.

**Design note — do NOT try to auto-detect limits.** An agent cannot reliably read its remaining weekly quota, and context-window limits are the harness's concern, not the CLI's. So handoff is NOT triggered by "sensing the edge". The robust design is the inverse: the orchestrator charter keeps the board CONTINUOUSLY current (a note/checkpoint after every meaningful action), so a valid handoff can be generated at ANY instant — including an abrupt cutoff — because nothing important was ever only in the agent's head. handoff carries what's written; the discipline is to always be writing. (This packet only adds the pre-flight nudge; the continuous-note discipline is enforced in roles/orchestrator.)
