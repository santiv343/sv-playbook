...
3. THE LINT LOCK (PRINCIPLE-011 single source, mechanized where possible): in eslint.config.js add to the typed block a no-restricted-syntax rule banning every domain literal outside its constants file. Build the entries by mapping over the list ['draft','ready','active','review','done','blocked','dropped','transition','note','takeover','evidence'] to objects { selector: "Literal[value='<v>']", message: "single source: use the constant from the module's .constants.ts" }. Then add an override AFTER the typed block that disables no-restricted-syntax for **/*.constants.ts and **/*.test.ts (constants files ARE the source; tests pin contracts deliberately). no-restricted-syntax and the existing no-restricted-properties are DIFFERENT rules - both stay. eslint.config.js joins the write_set.
