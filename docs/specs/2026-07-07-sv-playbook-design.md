...
**Durability rule:** SQLite is authoritative for operational state. Losing it is a serious recovery event, not a harmless cache miss. The system therefore uses local rotating backups and will grow an explicit backup command that checkpoints/copies the DB to a configured snapshot target. The default target is local and creates no Git noise. Optional remote targets, such as a dedicated backup branch, are adapters for users/projects that have permission and want off-machine durability; they are not a core requirement.

**Single-author flow (CLI-first ergonomics):** agents never hand-write operational state. Mutations go through CLI commands (`task create|move|note`, evidence capture, review actions) that validate at write time and update SQLite. Semantic markdown remains authored/reviewed as documentation, but it does not duplicate operational truth. **Safety rails (D26).** The store carries `PRAGMA user_version`; on open, a version mismatch refuses mutation with a documented recovery path. Every open also rotates a lightweight local backup copy (`.svp/backups/`, last 10).
...
| D26 | Safety-rail package: schema user_version refusal, explicit backup/restore path, rotating store backups, no-dead-ends principle | A worker stranded by a schema mismatch deleted the DB; day-1 telemetry lost | Prose prohibitions alone (proven insufficient the same day they were written) |
