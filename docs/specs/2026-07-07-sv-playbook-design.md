...
- **PRINCIPLE-010 — No dead ends.** Every error an agent can encounter must carry a documented, non-destructive exit: a recovery command, an automatic self-heal, or `blocked`. An agent improvising destructively around an unrecoverable error is a SYSTEM bug, not an agent failure — the fix is always a new rail, never a scolding. (Origin: a worker hit a schema mismatch with no recovery path and deleted the coordination DB, 2026-07-08.)
- **PRINCIPLE-011 — Single source for every fact.** Any value, type, schema, route, rule, or piece of knowledge exists in exactly ONE authored place; everything else references or derives from it. Duplicated unions, scattered domain literals, parallel lists, half-applied constants and copy-pasted config are all the same defect. Mechanized wherever possible (duplicate-string lint, restricted-syntax on domain literals, type derivation); everything the machine cannot catch is an INSTANT review failure — this rule cannot pass review violated. (User directive, 2026-07-08.)
- **PRINCIPLE-009 — Generated boilerplate, authored deltas.** Anything that repeats across documents or sessions is generated by the CLI or extracted to one source and referenced by ID — never rewritten by an agent. Agents author only the variable content (the delta). All authored documents follow fixed template structures with required sections; `check structure` validates it. Agent-facing writing is maximally concise: no restating the process, no repetition, reference by ID.
...
| Gates | verify + clean git | + strict lint, task states | All: check (all targets), doc-audit, arch, coverage |
...
- **Two layers:** a global profile (project-agnostic, travels with the user; imported by `init`) plus a per-project profile for that repo's specifics. Project overrides global. The global layer lives in the user's personal knowledge repo (configurable, e.g. `knowledge-hub`), which is also the home of cross-project distilled learnings — one personal hub, referenced by every project's config.
...
## 8. State architecture — SQLite operational truth + semantic documents

**Semantic document plane (markdown in git).** Long-form meaning lives in markdown: principles, role charters, specs, plans, ADRs, analysis, and any packet context meant to be reviewed by humans. Markdown is not the operational state database. Agents must not mirror statuses, leases, transitions, or evidence state into ad hoc markdown/JSON files.

**Operational plane (SQLite via `node:sqlite`, written ONLY by the CLI).** `.svp/playbook.sqlite` is the source of truth for work-management state: statuses, transitions with timestamps, owners, leases, agent sessions, events, notes, evidence records, sprints, and priority ordering. The DB is local and gitignored because SQLite is binary and not mergeable. `serve` and agents read this state through the CLI/materialized views, never by editing DB files by hand.

**Durability rule:** SQLite is authoritative for operational state. Losing it is a serious recovery event, not a harmless cache miss. The system therefore uses local-first state backups via the `backup state` command (checkpoints the DB to `.svp/backups/` with branch/SHA/sha256 metadata), with auto-backup on packet-close/takeover and an explicit `restore state` recovery path. The default target is local and creates no Git noise. Optional remote targets, such as a dedicated backup branch, are adapters for users/projects that have permission and want off-machine durability; they are not a core requirement.

**Single-author flow (CLI-first ergonomics):** agents never hand-write operational state. Mutations go through CLI commands (`task create|move|note`, evidence capture, review actions) that validate at write time and update SQLite. Semantic markdown remains authored/reviewed as documentation, but it does not duplicate operational truth. **Safety rails (D26).** The store carries `PRAGMA user_version`; on open, a version mismatch refuses mutation with a documented recovery path.
...
sv-playbook check <target>    # ALL verification under one self-describing verb:
                              ...
                              # metaphors ("grill" retired — it collided with the grill-me skill).
...
sv-playbook status            # stable board read model for serve and automation
sv-playbook backup state      # local-first SQLite snapshot + metadata; target adapters later
sv-playbook restore state     # explicit restore from a SQLite snapshot, after pre-restore backup
...
**Autonomy levels** (`autonomy` in `playbook.config.json`; overridable per packet and per model tier):

- `strict` — any deviation from plan/packet blocks. Default for cheap models and for new model/project pairings.
- `standard` — the agent may self-resolve a deviation only when ALL hold: (a) inside the declared write-set, (b) verifiable green by an existing gate, (c) reversible, (d) recorded as a `DEVIATION-xxx` entry in the evidence with rationale — reviewed by the human like any other claim. Example: a plan/tool-config contradiction with a documented-safe fix.
- `high` — may additionally amend plan *details* (never scope, write-set, or contract), same deviation record.

The invariant that keeps autonomy hallucination-safe: **autonomy widens what may be decided, never what may be claimed without proof.** Evidence duties are identical at every level; a deviation without its record is a contract violation regardless of how good the fix was. Escalation feeds learning: deviations that recur across packets become plan-template or checklist fixes.
...
- R5. Structural-check determinism boundary: semantic ambiguity detection stays human+agent judgment.
...
| D26 | Safety-rail package: schema user_version refusal, explicit backup/restore path, no-dead-ends principle (rotating open-backups dropped 2026-07-08 as redundant with `backup state`) | A worker stranded by a schema mismatch deleted the DB; day-1 telemetry lost | Prose prohibitions alone (proven insufficient the same day they were written) |
| D25 | Founder delegates merge to the reviewer verdict (this project): APPROVED = merge, post-hoc report | User stays out of code entirely; counterpart is the always-strict full-charter review | Asking per PR (friction without added safety once review is rigorous) |
