...
| 5b | EXEC | RED producibility — the expected-failure-cause string is chosen from a CLOSED list of two: (a) the compiler's missing-import/export message ("Cannot find module" / "has no exported member") when the test imports something new, or (b) THE TEST NAME (node:test always prints it on the failing line). NOTHING else — never assertion-content strings, never stderr text, never gate-rule names (run gates first and quote their real output only for gate packets) | The string is one of the two allowed forms | Rewrite. (Origin: the planner violated the open-ended version of this rule THREE times in one day: LINT-STRICT-001, STORE-GUARD-001, CLI-DOCTOR-001.) |
