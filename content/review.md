...
**Process-enforced:** the *independent approving review* is not mechanically
required — a single-token repo cannot force it via API (the author cannot
self-approve). A SEPARATE reviewer agent gives the verdict and **performs the
merge** on APPROVED (M1–M3 in `docs roles/reviewer`). `PLANNED`: mechanize the
approval with a second identity / bot token or CODEOWNERS. The mechanized floor
is the no-direct-push + status-check block; an agent that merges or pushes to
`main` without an APPROVED review violates the constitution.
